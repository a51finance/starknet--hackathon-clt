...
mod cltBase {
    ...
    impl CLTBaseImpl of super::ICLTBase<ContractState> {
        ...
        fn shift_liquidity(ref self: ContractState, params: ShiftLiquidityParams) {
            // Retrieve the strategy data for the given strategy ID
            let mut strategy = self.strategies.read(params.strategy_id);

            // Update global strategy fees
            let mut global = _update_globals(ref self, ref strategy, params.strategy_id);
            ...
            let mut vars = Account {
                ...
                balance1: 0.into(),
                total_shares: strategy.account.total_shares,
                jediswap_liquidity: strategy.account.jediswap_liquidity,
                fee_growth_inside0_last_x128: strategy.account.fee_growth_inside0_last_x128,
                fee_growth_inside1_last_x128: strategy.account.fee_growth_inside1_last_x128,
                fee_growth_outside0_last_x128: strategy.account.fee_growth_outside0_last_x128,
                fee_growth_outside1_last_x128: strategy.account.fee_growth_outside1_last_x128,
            ...
            // Burn liquidity specific to this strategy
            let (balance0, balance1, _, _) = PoolActions::burn_liquidity(
                strategy.key, vars.jediswap_liquidity
            );
            vars.balance0 = balance0;
            vars.balance1 = balance1;

            // Check the hodl status and adjust the global liquidity if needed
            // if strategy.get_hodl_status() == false {
            global.total_liquidity -= strategy.account.total_shares;
            // }

            // Retrieve the automation fee for governance
            let (automation_fee, _, _, _) = _get_governance_fee(@self, strategy.is_private);

            // Deduct protocol fees if necessary
            let (fee0, fee1) = CLTPayments::transfer_fee(
                strategy.key,
                0,
                automation_fee,
                vars.balance0,
            ...
            // No support for compound as of yet
            // // If the strategy is set to compound, add accumulated fees to balances
            // if strategy.is_compound {
            //     vars.balance0 += strategy.account.fee0;
            //     vars.balance1 += strategy.account.fee1;
            //     self.emit(FeeCompounded {
            //         strategy_id: params.strategy_id,
            //         fee0: strategy.account.fee0,
            //         fee1: strategy.account.fee1
            //     });
            // }

            // Add any unused assets to the balance
            ...
            let zero = super::IntegerTrait::<super::i256>::new(0, false);

            // Execute swap if a swap amount is specified
            if params.swap_amount != zero {
                let (amount0_swapped, amount1_swapped) = PoolActions::swap_token(
                    params.key.pool,
                    params.zero_for_one,
                    params.swap_amount,
                    params.sqrt_price_limit_x96,
                    _get_swap_callback_data(
                        token0, token1, pool_Dispatcher.get_fee(), params.zero_for_one
                    ),
                );

                let amount0_converted: super::i256 = if amount0_swapped < zero {
                    (-amount0_swapped).into() // Convert to positive and cast to u256
                } else {
                    amount0_swapped.into() // Directly cast to u256
                };

                let amount1_converted: super::i256 = if amount1_swapped < zero {
                    (-amount1_swapped).into() // Convert to positive and cast to u256
                } else {
                    amount1_swapped.into() // Directly cast to u256
                };

                let (new_balance0, new_balance1) = PoolActions::amounts_direction(
                    params.zero_for_one,
                    vars.balance0,
                    vars.balance1,
                    amount0_converted.mag,
                    amount1_converted.mag
                );

                vars.balance0 = new_balance0;
                vars.balance1 = new_balance1;
            }

            let mut liquidity_delta: u128 = 0;
            let mut amount0_added: u256 = 0.into();
            let mut amount1_added: u256 = 0.into();

            // Mint new liquidity if required
            if params.should_mint {
                let (liquidity_delta_new, amt0_added, amt1_added) = PoolActions::mint_liquidity(
                    strategy.key,
                    vars.balance0,
                    vars.balance1,
                    _get_mint_callback_data(
                        get_caller_address(), token0, token1, pool_Dispatcher.get_fee()
                    )
                );
                liquidity_delta = liquidity_delta_new;
                amount0_added = amt0_added;
                amount1_added = amt1_added;
            }

            // Update strategy state, impacting all users associated with this strategy ID
            Position::update_strategy(
                ref self,
                ref strategy,
                params.key,
                params.module_status,
                liquidity_delta,
                vars.balance0 - amount0_added,
                vars.balance1 - amount1_added
            );
            // Emit the LiquidityShifted event
            self
                .emit(
                    LiquidityShifted {
                        strategy_id: params.strategy_id,
